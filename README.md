...
Este proyecto es una solución integral para el procesamiento, limpieza y análisis de logs de alarmas industriales (SCADA/Gateway). La arquitectura está diseñada bajo principios de ingeniería de software moderna, permitiendo la transición de scripts aislados a una API robusta y contenedorizada.

## 1. Descripción de la Solución

El sistema implementa un pipeline ETL (Extract, Transform, Load) que:
1.  **Ingesta:** Recibe archivos de logs brutos (CSV/Parquet).
...
* **Capa de Dominio (Business Logic):** Contiene la lógica de limpieza y procesamiento (Clase `AlarmCleaner`). Se separó de la API para permitir su uso en scripts de consola o procesos batch.
...
* **Python + Pandas:** Se eligió para la fase de limpieza por su eficiencia en el manejo de estructuras vectorizadas, ideal para logs industriales masivos.
* **MySQL 8.0:** Proporciona un motor de persistencia robusto y estándar en la industria.
* **Validación de Datos (Supuestos):**
    * Se asume que los timestamps deben ser cronológicos; registros con tiempo invertido o nulo son descartados.
    * Se implementó una lógica de "deduplicación agresiva" asumiendo que eventos idénticos en el mismo segundo son errores de comunicación del Gateway.
